...
export default function Layout({ currentPage, setPage, children }: Props) {
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'linear-gradient(135deg,#0f0c29 0%,#1a1040 50%,#0d1b2a 100%)' }}>
      ...
      {/* BG orbs */}
      {[
        { w:380, h:380, bg:'rgba(124,58,237,0.35)',  top:-90,  left:-90,  delay:'0s'  },
        { w:280, h:280, bg:'rgba(6,182,212,0.25)',   bottom:-50,right:-50,delay:'-4s' },
      ].map((o, i) => (
        <div key={i} style={{
          position:'fixed', width:o.w, height:o.h, borderRadius:'50%',
          background:`radial-gradient(circle,${o.bg},transparent)`,
      ...
      <nav style={{
        ...
        padding: mini ? '16px 6px' : '18px 10px',
        ...
        <div style={{
          ...
          <button onClick={() => setMini(m => !m)} title={mini ? 'Expand' : 'Collapse'} style={{
            ...
            justifyContent:'center', cursor:'pointer', fontSize:11, flexShrink:0,
          }}>
            {mini ? '▶' : '◀'}
          </button>
        </div>
